...
> **Run this check at the START of every session before doing anything else.**

1. **Check for an existing context snapshot**:
   - Look for files in `.agents/context-snapshots/` (newest first).
   - If a snapshot exists for the current task, **read it now** and skip to the step listed in "Remaining Work".
   - Do NOT re-read files already summarised in the snapshot.

2. **Load minimal context**:
   - Load `.agents/project-profile.json` if it exists (auto-generated by `agents-skills init`).
   - Load ONLY the skills needed for **this specific task** — not all skills. Default: `code-reviewer` + `git-workflow` + the task-specific skill.
   - Each skill costs ~5,000–10,000 tokens. Loading all 20 skills uses ~150,000 tokens — 75% of a Claude Sonnet session budget alone.

3. **Budget status**:
   - Claude Sonnet/Opus 4: context budget = **80,000 tokens** (40% of 200k)
   - GPT-4o: context budget = **51,200 tokens** (40% of 128k)
   - Run `agents-skills tokens --budget` at any point to see live usage.

4. **Mid-session compaction trigger**:
   - If you have read > 15 files OR the session has been running many turns, create a snapshot before continuing:
     - Run `agents-skills compact` in the terminal, OR
     - At the end of each phase, write a brief checkpoint note internally ("Phase 2 complete: plan approved, will modify X files")
...
| -------------------------------------- | ----------------------------------------------------- |
| _"Add a new feature..."_               | Check Phase 0 snapshot → Run Phases 1–4               |
| _"Fix this bug..."_                    | Use `debug-investigator` skill, then Phases 1–4       |
...
| _"Check context / tokens"_             | Run `agents-skills tokens --budget` in terminal       |
| _"Save my progress / compact"_         | Run `agents-skills compact` in terminal               |
| _"Continue from last time"_            | Read snapshot in `.agents/context-snapshots/` first   |
