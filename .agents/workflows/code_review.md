---
description: Run a systematic, PR-style Code Review on recent changes
---

# Code Review Agent Workflow

**Purpose**: Act as an expert code review agent to provide thorough, constructive, and actionable feedback on code changes.
**Why it exists**: To ensure all merged code meets strict project standards, is secure, performant, and well-tested before finalizing tasks.
**Token Optimization Strategy**: Structured, step-by-step evaluation focusing only on files modified in the current context.

**INSTRUCTIONS**: Use this workflow when a code review is requested. Evaluate the recent changes according to the principles below and output a structured Review Feedback report.

## Code Review Principles

Before providing any review feedback, you must methodically analyze the modified code against the following criteria:
...
4.1) Are there N+1 queries or unnecessary API/database calls?
4.2) Are expensive operations optimized or cached (e.g., `useMemo`, `useCallback` in React)?
4.3) Is there proper pagination for large datasets?
4.4) Are there memory leaks (e.g., missing `useEffect` cleanup) or resource cleanup issues?
...
8.2) Are public APIs exported and typed correctly?
...
After analysis, output your review precisely using this format. Do not combine issues; list them separately.

**Review Feedback:**

- **Severity**: 🔴 Critical | 🟠 Important | 🟡 Suggestion | 💡 Nitpick
- **Location**: `[File Name.ts]` (Line X)
- **Issue**: [Clear description of the problem]
- **Suggestion**: [Specific recommendation for improvement]
- **Example**:

```tsx
// Insert code snippet showing the fix if applicable
```
