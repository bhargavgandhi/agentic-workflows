...
### In Scope (v3)
- New skill taxonomy: process skills (mandatory + optional) + tech skills (optional)
- 12 new/upgraded mandatory process skills
- 5 new optional process skills
- 10 existing tech skills upgraded to 7-section anatomy
- Tech packs as install-time aliases (not versioned units)
- Master workflow + individual slash commands (Model C)
- Skill gap detection — proactive Model B (pre-task audit)
...
│  Mandatory (12) + Optional (5) — installed by default           │
├─────────────────────────────────────────────────────────────────┤
...
### 7a. Mandatory Process Skills (12)

These ship in the core bundle. Every install gets them.
...
### 7b. Optional Process Skills (5)

Installed on demand. Master workflow offers them as opt-in gates.
...
`/build-feature` — chains all mandatory phases with gate conditions between each.
...
## 11. Skill Gap Detection (Model B — Proactive)

Before any implementation begins, the master workflow runs a pre-task audit:

1. Parse the task description and PRD to extract technology signals (e.g., "Firebase", "GraphQL", "React Native")
2. Check installed skills against `skills.json` registry manifest
3. Cross-reference against public registry (GitHub repo) for available matches
4. Surface one of three outcomes:

```
OUTCOME A — All skills present:
  "All required skills are installed. Proceeding."

OUTCOME B — Skills available in registry:
  "This task requires [react-native, graphql-frontend].
   Neither is installed. Options:
   1. Install now: agents-skills install react-native graphql-frontend
   2. Install react-pack (includes both): agents-skills install react-pack
   3. Proceed without (agent uses training data — lower confidence)"

OUTCOME C — Skills not in registry:
  "This task requires [some-new-framework].
   No matching skill found in registry.
   Options:
...
agents-skills upgrade
  ...
  STEP 3 — Execute (only after confirmation):
    ...
    → Install new mandatory process skills
...
Author in this priority order (mandatory first, high-leverage first):
...
1. Author `/build-feature` master workflow
2. Author individual slash commands for each phase
3. Implement skill gap detection (Model B) as Phase 0 of master workflow
4. Wire optional gates (perf, docs) as user opt-in prompts

### Phase E — Validation
1. End-to-end test: `npx agents-skills install` on a fresh workspace
2. Run `/build-feature` on a real feature (internal team dogfood)
3. Verify skill gap detection surfaces correct registry matches
4. Verify agent-driven compact produces valid snapshots
5. Publish v3 to npm
...
|----------|---------|-------------------|
| Should `app-architect` be retired or merged into `write-a-prd`? | Both do requirements gathering, but `app-architect` is architecture-focused, `write-a-prd` is product-focused. Could keep both with distinct triggers. | Phase B kickoff |
| Should `doc-coauthoring` be renamed `documentation-and-adrs`? | Current skill covers co-authoring docs generically. v3 wants ADR-specific guidance. Could extend or rename. | Phase C kickoff |
| How does skill gap detection handle skills not in the public registry? | Outcome C proposes `/skill-creator`. Does the agent auto-invoke it or just suggest it? | Phase D kickoff |
